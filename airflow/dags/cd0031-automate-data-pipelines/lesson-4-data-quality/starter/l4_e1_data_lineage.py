...
def data_lineage():
    ...
    drop_station_traffic_table_task = PostgresOperator(
...
# Done: First, load the Airflow UI and run this DAG once.
# Done: Next, add the calculate_traffic_task to the end of the flow and run it again
# Done: Finally, configure the task ordering for the stations table to be created before 
#       loading station data. Keep in mind these two tasks can run independently of 
#       the others.
# Added a step to drop station_traffic table before its creation

    load_trip_data_to_redshift_task >> calculate_traffic_task

    create_stations_table >> load_station_data_to_redshift_task
    
    load_station_data_to_redshift_task >> drop_station_traffic_table_task

    drop_station_traffic_table_task >> calculate_traffic_task
